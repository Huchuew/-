import type { RiftCombatSetup } from '../systems/RiftSystem';
...
/** 차원 균열 10초 미니 전투 — 자동 DPS + 투닥 화력 */
export function runRiftCombatOverlay(
  setup: RiftCombatSetup,
  onComplete: (result: RiftCombatResult) => void,
): Promise<void> {
  return new Promise(resolve => {
    const root = document.createElement('div');
    root.className = 'rift-combat-overlay';
    root.innerHTML = `
      <div class="rift-combat-card">
        <h3>🌌 ${setup.floorName}</h3>
        <p class="rift-combat-sub">${setup.elementIcon} ${setup.zone} · ${COMBAT_DURATION_SEC}초 격파</p>
        <div class="rift-enemy-hp">
          <div class="rift-enemy-hp-fill" style="width:100%"></div>
        </div>
        <p class="rift-hp-label">적 HP <span class="rift-hp-num">${setup.enemyHp.toLocaleString()}</span></p>
        ...
        <p class="rift-dmg-label">누적 피해 <span class="rift-dmg-num">0</span></p>
        <button type="button" class="rift-tap-btn">투닥!</button>
        <p class="hint">자동 DPS + 투닥으로 추가 화력</p>
      </div>`;
